...
func _ready() -> void:
	...
	plaster = textured("clay_plaster",Color(.88,.84,.73),.38)
...
func build_main_house() -> void:
	...
	for level in 3:
		...
		for side in [-1.0,1.0]: side_facade(h,y,side)
		interior(h,level)
		for x in [-32.0,32.0]:
...
func side_facade(h: Node3D,y: float,side: float) -> void:
	for i in 6:
...
func window(h: Node3D,p: Vector3,extent: Vector2,along_x: bool) -> void:
	...
	piece(h,"WindowMullion",p,mullion,wood,false)
	piece(h,"WindowTransom",p,Vector3(extent.x,.1,.13) if along_x else Vector3(.13,.1,extent.x),wood,false)
	for offset in [-.27,.27]:
		var shutter := p+Vector3(offset*extent.x,0,.12) if along_x else p+Vector3(.12,0,offset*extent.x)
		piece(h,"TimberShutter",shutter,Vector3(extent.x*.38,extent.y,.09) if along_x else Vector3(.09,extent.y,extent.x*.38),wood,false)
...
func interior(h: Node3D,level: int) -> void:
	...
	for side in [-1.0,1.0]:
		for z in [-11.0,11.0]:
			piece(h,"RoomPartition",Vector3(side*17.5,y+2.2,z),Vector3(.35,4.4,14),plaster)
		for x in [-24.0,-10.0,10.0]:
			piece(h,"CrossPartition",Vector3(x,y+2.2,0),Vector3(10,4.4,.35),plaster)
		piece(h,"CrossDoorLintel",Vector3(side*17.5,y+3.8,0),Vector3(.35,1.2,7.8),plaster)
